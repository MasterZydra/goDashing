...
env GOOS=$os GOARCH=$arch packr build -ldflags="-s -w -X main.version=${1} -X main.buildstamp=`date -u '+%Y-%m-%d_%I:%M:%S%p'` " -o $product ./cmd/godashing/...
...
os=windows
product=${productName}_${versionLabel}_${os}_${arch}
env GOOS=$os GOARCH=$arch packr build -ldflags="-s -w -X main.version=${1} -X main.buildstamp=`date -u '+%Y-%m-%d_%I:%M:%S%p'` " -o ${product}.exe ./cmd/godashing/...
